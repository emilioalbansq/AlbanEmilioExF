...
public class PSexo extends JPanel implements ActionListener {
    private transient FactoryBL<aeSexoDTO> blFactory = new FactoryBL<>(aeSexoDAO.class);
    private transient aeSexoDTO dtoSexo = new aeSexoDTO();
    ...
    private void showRowData() {
        boolean isDTONull = (dtoSexo == null || dtoSexo.getIdCatalogo() == null);
        txtIdSexo.setText   ((isDTONull) ? " " : dtoSexo.getIdCatalogo().toString());
    ...
    private void showDataTable() throws Exception {
        ...
        int index = 0;
        for (aeSexoDTO  d : blFactory.getAll()) {
            data[index][0] = d.getIdCatalogo();
    ...
    private void btnGuardarClick() {
        ...
        try {
            if (AppMSG.showConfirmYesNo("¿Seguro que desea " + ((isDTONull) ? "AGREGAR ?" : "ACTUALIZAR ?"))) {
                if (isDTONull)  
                    dtoSexo = new aeSexoDTO(txtNombre.getText().trim(),"");
                else
    ...
    private void btnEliminarClick() {
        try {
            if (AppMSG.showConfirmYesNo("Seguro que desea Eliminar?")) {
                ...
                if (!blFactory.del(dtoSexo.getIdCatalogo()))
